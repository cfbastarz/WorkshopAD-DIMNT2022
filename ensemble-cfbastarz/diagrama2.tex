...
\node (bam_det_prev_hib) [process7, below of=fct_rdp_numn, visible on=<2->] 
...
% Flechas (col2)
\draw [stealth-stealth, visible on=<2->] (fct_ctr_ncep) -- node[anchor=west] {EOF} (fct_rdp_num1);
\draw [path, dashed, visible on=<2->] (fct_rdp_num1) -- node[anchor=west] {EOF} (fct_rdp_num2);
\draw [path, dashed, visible on=<2->] (fct_rdp_num2) -- node[anchor=west] {EOF} (fct_rdp_dots);
\draw [path, dashed, visible on=<2->] (fct_rdp_dots) -- node[anchor=west] {EOF} (fct_rdp_numn);
\draw [stealth-stealth, visible on=<2->] (fct_rdp_numn) -- node[anchor=west] {EOF} (bam_det_prev_hib);
...
\draw[thick, dotted, visible on=<7->] ($(fct_rdp_num1.north west)+(-0.2,0.45)$) rectangle ($(bam_det_prev_hib.south east)+(0.16,-0.15)$);

\draw[thick, dotted, visible on=<7->] ($(fct_rdp_num1.north west)+(6.5,1.1)$) rectangle ($(bam_det_prev_hib.south east)+(7.15,1.55)$);
\node (r1Label)[above=-0.75cm of fct_ctr_ncep2, visible on=<7->] {\begin{varwidth}{3cm}\Centering Prev. Ensemble\\(15 dias) \end{varwidth}};

\draw[thick, dotted, visible on=<7->] ($(fct_rdp_num1.north west)+(6.5,-3.75)$) rectangle ($(bam_det_prev_hib.south east)+(7.15,-0.75)$);
\node (r2Label)[above=-0.1cm of fct_eofn, visible on=<7->] {\begin{varwidth}{3.5cm}\Centering Prev. Determinística\\(7 dias) \end{varwidth}};
